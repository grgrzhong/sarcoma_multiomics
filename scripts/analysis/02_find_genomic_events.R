...
mat_list <- list(
    ...
    gistic_cnv_cytoband = gistic_cn_cytoband_mat
)
...
## "========================================================================="
## Oncoplot shared events across groups- PCGR ----
## "========================================================================="
### Load data ----
stat_res_list <- LoadData(
    dir = "data/processed",
...
sarcoma_cnv_gene <- sarcoma_data$cna_gene_data |> 
    dplyr::filter(freq > cnv_gene_freq_thres) |> 
    pull(symbol)
...
snv_indel_gene_cohort <- stat_res_list$freq$pcgr_mutated_gene |>
...
snv_indel_gene_list <- pcgr_snv_indel_freq |> 
...
cnv_cytoband_freq <- stat_res_list$freq$pcgr_cnv_cytoband |> 
...
cnv_facet_gene_freq <- stat_res_list$freq$cnv_gene_bedtools |> 
...
VerticalCombinedOncoplot(
    snv_indel_mat = pcgr_snv_indel_mat,
...
## "========================================================================="
## Oncoplot Pre-FST vs Post-FST - PCGR ---- 
## "========================================================================="
...
share_cytobands <- stat_res_list$share$pcgr_cnv_cytoband[["Pre-FST_vs_Post-FST"]] |> 
    arrange(desc(group2_freq)) |> 
    filter(group1_freq > 0.3 & group2_freq > 0.3) |> 
    mutate(cytoband = paste0(cytoband, "_", alteration_type)) |>
    pull(cytoband)

diff_cytobands <- stat_res_sig$pcgr_cnv_cytoband[["Pre-FST_vs_Post-FST"]] |> 
    arrange(desc(group2_freq)) |> 
    mutate(cytoband = paste0(cytoband, "_", alteration_type)) |>
    pull(cytoband)
...
cnv_gene_share <- stat_res_list[["share"]][["pcgr_cnv_gene"]][["Pre-FST_vs_Post-FST"]] |> 
    filter(group1_freq > 0.3 & group2_freq > 0.3) |>
    arrange(desc(group2_freq))

cnv_gene_diff <- stat_res_sig[["pcgr_cnv_gene"]][["Pre-FST_vs_Post-FST"]] |> 
...
## "========================================================================="
## Oncoplot U-DFSP vs FS-DFSP - PCGR ------
## "========================================================================="
pcgr_cn_cytoband_mat <- GetVariantTypeMat(
    data = pcgr_cnv_tbl,
    variant_type = "cytoband",
...
gc <- "U-DFSP_vs_FS-DFSP"

share_cytobands <- stat_res_list$share$pcgr_cnv_cytoband[[gc]] |> 
    # filter(group1_freq > 0.4 & group2_freq > 0.4) |> 
    mutate(cytoband = paste0(cytoband, "_", alteration_type)) |>
    arrange(desc(group2_freq)) |>
    pull(cytoband)

diff_cytobands <- stat_res_sig$pcgr_cnv_cytoband[[gc]] |> 
    mutate(cytoband = paste0(cytoband, "_", alteration_type)) |>
    # arrange(desc(group2_freq)) |> 
...
cnv_cytoband_mat <- pcgr_cn_cytoband_mat[show_cytobands, ]

ComplexGroupOncoplot(
    mat = cnv_cytoband_mat,
    is_somatic_matched = TRUE,
    split_by_group = "FST.Group",
    sort_group_level = c("U-DFSP", "FS-DFSP"),
    main_group = "FS-DFSP",
    row_order = show_cytobands,
    sample_annotation = c("FST.Group", "Metastasis", "Specimen.Nature"),
    column_title = paste0("PCGR CNV Cytoband - ", gc),
    width = 10.5,
    height = 8,
    dir = "figures/oncoplot",
    filename = paste0("wes_cnv_facets_pcgr_cn_cytoband_", gc)
)

## "========================================================================="
## Oncoplot significant diff events across groups - PCGR ----
## "========================================================================="
...
## "========================================================================="
## Oncoplot Pre-FST vs Post-FST - Gistic2 ---- 
## "========================================================================="
gistic_data <- LoadData(
    dir = "data/processed",
    filename = "wes_cnv_facets_gistic2_somatic_matched_all_tumors_qval0.5"
...
cnv_cytoband_mat <- gistic_cn_cytoband_mat[show_cytobands, ]

case_event_diff <- cnv_cytoband_mat[show_cytobands, ] |> 
    as_tibble(rownames = "cytoband") |> 
    pivot_longer(
        -cytoband,
        names_to = "Sample.ID",
    ...
    left_join(LoadClinicalInfo(), by = "Sample.ID") |> 
    # filter(cytoband %in% diff_cytobands) |> 
    filter(FST.Group  %in% c("Pre-FST", "Post-FST")) |> 
    dplyr::filter(Number.of.samples.per.patient >= 2) |> 
    group_by(Case.ID, cytoband) |> 
...
## Plot the oncoplot
PairedPatientGroupOncoplot(
    mat_list = mat_list,
    is_somatic_matched = TRUE,
    paired_group = c("Pre-FST", "Post-FST"),
    column_sort = c("Case.ID", "FST.Group"),
    sample_annotation = c("FST.Group", "Metastasis", "Specimen.Nature"),
    column_title = paste0("GISTIC CNV Cytoband - ", gc),
    title_size = 7,
    text_size = 6,
    width = 10,
    height = 8,
    dir = "figures/oncoplot",
    filename = "wes_cnv_facets_gistic_cnv_cytoband_Pre-FST_vs_Post-FST_oncoplot"
)

## "========================================================================="
## Oncoplot U-DFSP vs FS-DFSP - Gistic2 ------
## "========================================================================="
gc <- "U-DFSP_vs_FS-DFSP"

# stat_res_list$diff$gistic_cnv_cytoband[[gc]] |> 
#     filter(grepl(":1p", cytoband))
...
ComplexGroupOncoplot(
    ...
    height = 8,
    dir = "figures/oncoplot",
    filename = paste0("wes_cnv_facets_gistic_cnv_cytoband_", gc)
)
