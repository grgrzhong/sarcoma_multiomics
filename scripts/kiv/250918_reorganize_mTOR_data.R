...
## Reorganize mTOR Sanger sequencing data ------------
...
## Summarize the data ------------
data_dir <- "/mnt/s/Dataset/Sanger Sequencing/SFT mTOR"
dir_list <- dir_ls(
    here(data_dir, "SFT mTOR Sanger Data"), 
    type = "directory"
)
dir_list <- dir_list[!grepl("riginal", dir_list, ignore.case = TRUE)]

sample_check_data <- NULL

for (dir in dir_list) {
    case <- basename(dir)
    files <- dir_ls(dir, type = "file")
    ## detect whethere there are mTOR1 files
    is_mtor1 <- any(grepl("mTOR1", files, ignore.case = TRUE))
    is_mtor2 <- any(grepl("mTOR2", files, ignore.case = TRUE))
    is_mtor3 <- any(grepl("mTOR3", files, ignore.case = TRUE))

    sample_check_data <- rbind(
        sample_check_data,
        tibble(
            Case.ID_Sanger = case,
            `mTOR-1 Sanger Result` = if_else(is_mtor1, "Yes", "No"),
            `mTOR-2 Sanger Result` = if_else(is_mtor2, "Yes", "No"),
            `mTOR-3 Sanger Result` = if_else(is_mtor3, "Yes", "No")
        )
    )

}

## Load the master project
master_project <- "SFT - Master Project"
master_project <- read_xlsx(
    here(data_dir, "SFT - Master Project .xlsx")
)
colnames(master_project)

checked_project <- master_project |> 
    select(
        -matches("Sanger Result")
    ) |> 
    left_join(
        sample_check_data,
        by = "Case.ID_Sanger"
    ...
    relocate(
        starts_with("mTOR-1 Sanger"), .after = starts_with("mTOR-1 Gel")
    ) |> 
    relocate(
        starts_with("mTOR-2 Sanger"), .after = starts_with("mTOR-2 Gel")
    ) |>
    relocate(
        starts_with("mTOR-3 Sanger"), .after = starts_with("mTOR-3 Gel")
    )
